...
// ImmoValue — réutilise EXACTEMENT la DA d'apps/web/tailwind.config.ts.
// Stone warm-neutral + violet brand + accent terra (Immovalue).
// Mode clair uniquement. Source : design system handoff Claude Design.
//
// Important : on inclut `../web/src/**/*.{ts,tsx}` dans `content` pour que
// Tailwind voie les classes utilisées par les composants ui/lib partagés
// (importés via l'alias `@web/*`).
...
export default {
  ...
  content: [
    "./index.html",
    "./src/**/*.{ts,tsx}",
    // Composants ui + lib partagés depuis apps/web (alias @web/*).
